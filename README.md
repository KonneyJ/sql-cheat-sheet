...
**HAVING** - оператор фильтрует значения полученные в результате группировки  
**ORDER BY** - оператор сортирует значения по возрастанию или убыванию
...
## Как группировать данные по категориям. Оператор GROUP BY
...
Группировать данные можно сразу по нескольким полям. Произойдет наложение - для каждого поля и для каждой категории скидки будет рассчитан средний возраст клиента.
```sql
...
## Как отсортировать данные. Оператор ORDER BY
...
По умолчанию оператор **ORDER BY** сортирует значения от меньшего к большему. Чтобы изменить порядок сортировки, после называния поля указывают ключевое слово **DESC**: тогда данные будут отсортированы по убыванию. Если указать **ASC** - данные будут отсортированы по возврастанию так же, как и по умолчанию.

В запросах можно указывать псевдонимы, чтобы не писать агрегирующие фукнции дважды.
```sql
SELECT connection_area,
       AVG(age)
FROM buyer
...
Сортировать по нескольким полям сложнее, чем группировать. При сортировке важен порядок полей, который указывают после оператора **ORDER BY**.  
Сортировки разных полей не конфликтуют между собой. Ни одно правило сортировки не нарушает предыдущего.  
Порядок, в котором указаны поля после оператора **SELECT**, влияет только на расположение полей в таблице.  
Если после **SELECT** указать поля в другой последовательности, но сохранить порядок сортировки после **ORDER BY**, поля поменяются местами в таблице, но порядок сортировки сохранится.
